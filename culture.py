    ...
    def simulate(self, save_progress=0, legend=''):
        if self.simulated:
            ...
            for i in range(self.maxiter):
                if save_progress:
                    if i % save_progress == 0: 
                        self.plot_gsnet(to_file=True, file_name=f'sim_{i}.png')
                    
                #    update_progress(i/self.maxiter)
            ...
            if (save_progress) and (self.usediter < self.maxiter):
                self.plot_gsnet(to_file=True, file_name='sim_final.png')
            self.simulated = True
            #if progress:
            #    update_progress(1)
...
class Simulation():
    ...
    def agents_regs_plot(self, f_cnt=6, t_cnt=10):
        ...
        ax = f.groupby('agents')['regions'].mean().plot(figsize=(10,5))
        ax.set_xlabel('počet agentů')        
        ax.set_ylabel('počet regionů')
